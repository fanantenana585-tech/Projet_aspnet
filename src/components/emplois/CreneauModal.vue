...
<template>
  <Transition name="modal">
    <div v-if="isOpen" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <!-- Modal Card -->
      <div class="bg-white w-full max-w-[950px] rounded-[2.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.25)] relative overflow-hidden flex flex-col max-h-[92vh] animate-modal-in">
        ...
        <div class="p-8 border-b border-[#F0F7FF] flex justify-between items-center bg-gradient-to-r from-[#F8FBFF] to-white shrink-0">
          <div class="flex items-center gap-6">
            ...
            <div>
              ...
              <p class="text-[10px] font-black text-[#64A8CC] uppercase tracking-[0.2em] mt-0.5">Planning Académique EMIT</p>
            </div>
          </div>
          <div class="flex items-center gap-4">
            ...
            <button @click="emit('close')" class="p-3 bg-white rounded-2xl text-[#64A8CC] hover:text-red-500 shadow-sm border border-[#BFDBFE] transition-all hover:rotate-90">
              <X :size="20" />
            </button>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar">
          ...
          <section class="space-y-8 animate-fade-slide-up">
            <div class="flex items-center gap-4 border-l-4 border-[#38BDF8] pl-4">
              ...
              <h3 class="text-sm font-black text-[#0C2340] uppercase tracking-widest">Informations du cours</h3>
            </div>
              ...
              <div class="space-y-2 animate-fade-slide-down" :key="form.mention">
                ...
                <div class="relative">
                   <button
                     ...
                     class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] rounded-2xl p-4 flex items-center justify-between font-bold text-[#0C2340] hover:border-[#38BDF8] transition-all"
                   >
                      ...
                      <ChevronDown :size="20" class="text-[#64A8CC]" />
                   </button>

                   <div v-if="showParcoursList" class="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-[#BFDBFE] rounded-2xl shadow-2xl z-[60] overflow-hidden animate-fade-slide-down">
                      <div
                        v-for="p in filteredParcours" :key="p.id"
                        @click="selectParcours(p)"
                        class="p-4 hover:bg-[#F0F9FF] cursor-pointer flex items-center gap-3 border-b border-[#F0F7FF] last:border-0 group"
                      >
                         ...
                         <span class="text-xs font-bold text-[#0C2340]">{{ p.nom }}</span>
                      </div>
                   </div>
                </div>
              </div>
            ...
            <div class="grid grid-cols-2 gap-8">
               ...
               <div class="space-y-2 animate-fade-slide-down" :key="form.parcours">
                ...
                <div class="flex bg-[#F0F9FF] p-1.5 rounded-2xl border-2 border-[#BFDBFE]">
                  <button
                    ...
                    :class="form.niveau === lvl ? 'bg-[#38BDF8] text-white shadow-md' : 'text-[#64A8CC] hover:text-[#0C2340]'"
                  >
                    {{ lvl }}
                  </button>
                </div>
              ...
              <!-- Groupe Dropdown -->
              <div class="space-y-2">
                <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Groupe</label>
                <div class="relative">
                   <select v-model="form.groupe" class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] text-[#0C2340] font-bold rounded-2xl p-4 appearance-none outline-none focus:border-[#38BDF8] transition-all">
                     <option :value="null">Tous</option>
                     <option>Groupe A</option>
                     <option>Groupe B</option>
                   </select>
                   <Users class="absolute right-4 top-1/2 -translate-y-1/2 text-[#64A8CC] pointer-events-none" :size="18" />
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-8">
               <div class="col-span-2 space-y-2 relative animate-fade-slide-down" :key="form.parcours + form.niveau">
                 <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Matière</label>
                 <div class="relative">
                    ...
                    <input
                      ...
                      class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] text-[#0C2340] font-bold rounded-2xl pl-12 pr-4 py-4 focus:border-[#38BDF8] outline-none transition-all"
                    >
                    <div v-if="showMatiereList && filteredMatieres.length > 0" class="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-[#BFDBFE] rounded-2xl shadow-2xl z-[50] max-h-60 overflow-y-auto custom-scrollbar">
                       <div
                         v-for="m in filteredMatieres" :key="m.id"
                         @click="selectMatiere(m)"
                         class="p-4 hover:bg-[#F0F9FF] cursor-pointer flex items-center justify-between border-b border-[#F0F7FF]"
                       >
                          <div class="flex flex-col">
                             <span class="text-sm font-black text-[#0C2340]">{{ m.nom }}</span>
                             <span class="text-[10px] font-bold text-[#64A8CC] uppercase">{{ m.code }}</span>
                          </div>
               ...
               <!-- Type Pills -->
               <div class="space-y-2">
                 <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Type</label>
                 <div class="flex bg-[#F0F9FF] p-1.5 rounded-2xl border-2 border-[#BFDBFE] overflow-x-auto no-scrollbar">
                   <button
                     ...
                     :class="form.type === t ? 'bg-[#38BDF8] text-white shadow-md' : 'text-[#64A8CC] hover:text-[#0C2340]'"
                   >
                     {{ t }}
                   </button>
                 </div>
          ...
          <section class="space-y-6 animate-fade-slide-up" style="animation-delay: 0.1s">
            <div class="flex items-center justify-between">
               <div class="flex items-center gap-4 border-l-4 border-[#0EA5E9] pl-4">
                 ...
                 <h3 class="text-sm font-black text-[#0C2340] uppercase tracking-widest">Intervenant</h3>
               </div>
               <div class="flex items-center gap-2">
                  <span class="text-[10px] font-black text-[#64A8CC] uppercase">Charge:</span>
            ...
            <div class="space-y-4">
               <div class="relative">
                  ...
                  <input
                    ...
                    class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] text-[#0C2340] font-bold rounded-2xl pl-12 pr-4 py-4 focus:border-[#38BDF8] outline-none transition-all"
                  >
                  <div v-if="showEnseignantList && filteredEnseignants.length > 0" class="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-[#BFDBFE] rounded-2xl shadow-2xl z-[50] max-h-60 overflow-y-auto custom-scrollbar">
                     <div
                        v-for="e in filteredEnseignants" :key="e.id"
                        @click="selectEnseignant(e)"
                        class="p-4 hover:bg-[#F0F9FF] cursor-pointer flex items-center gap-4 border-b border-[#F0F7FF]"
                     >
                        ...
                        <div class="flex flex-col">
                           <span class="text-sm font-black text-[#0C2340]">{{ e.prenom }} {{ e.nom }}</span>
                           <span class="text-[9px] font-bold text-[#64A8CC]">{{ e.email }}</span>
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div class="space-y-2">
                  <div class="h-2 w-full bg-[#F0F9FF] rounded-full overflow-hidden border border-[#BFDBFE]/50 relative">
                     <div
          ...
          <section class="space-y-8 animate-fade-slide-up" style="animation-delay: 0.2s">
            <div class="flex items-center gap-4 border-l-4 border-[#059669] pl-4">
              ...
              <h3 class="text-sm font-black text-[#0C2340] uppercase tracking-widest">Lieu et Temps</h3>
            </div>

            <div class="space-y-8">
               <div class="flex gap-2 p-1.5 bg-[#F0F9FF] rounded-[1.5rem] border-2 border-[#BFDBFE]">
                  <button
                    ...
                    :class="form.jour === j ? 'bg-white text-[#38BDF8] shadow-md border border-[#BFDBFE]' : 'text-[#64A8CC] hover:text-[#0C2340]'"
                  >
                    {{ j }}
                  </button>
               </div>
               ...
               <div class="grid grid-cols-3 gap-8 items-end">
                  <div class="col-span-2 grid grid-cols-2 gap-6">
                     <div class="space-y-2">
                        <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Début</label>
                        <input type="time" v-model="form.heureDebut" class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] rounded-2xl p-4 font-black text-[#0C2340] focus:border-[#38BDF8] outline-none">
                        <div class="flex flex-wrap gap-2 mt-2">
                           <button v-for="s in ['08:00', '10:30', '14:00', '16:00']" :key="s" @click="form.heureDebut = s" class="text-[9px] font-black bg-white border border-[#BFDBFE] px-2 py-1 rounded-lg text-[#64A8CC] hover:text-[#38BDF8] hover:border-[#38BDF8] transition-all">{{ s }}</button>
                        </div>
                     </div>
                     <div class="space-y-2">
                        <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Fin</label>
                        <input type="time" v-model="form.heureFin" class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] rounded-2xl p-4 font-black text-[#0C2340] focus:border-[#38BDF8] outline-none">
                        <p class="text-[10px] font-black text-[#64A8CC] text-right mt-1">Durée : <span class="text-[#38BDF8]">{{ dureeLabel }}</span></p>
                     </div>
                  </div>
                  <div class="space-y-2">
                     <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Salle</label>
                     <div class="relative">
                        <select v-model="form.salle" class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] text-[#0C2340] font-bold rounded-2xl p-4 appearance-none outline-none focus:border-[#38BDF8]">
                           <option :value="null">Sélectionner...</option>
                           <option v-for="s in Object.values(SALLES)" :key="s.id" :value="s">
                              {{ s.nom }} - {{ s.batiment }} ({{ s.capacite }}p)
                           </option>
                        </select>
                        <MapPin class="absolute right-4 top-1/2 -translate-y-1/2 text-[#64A8CC] pointer-events-none" :size="18" />
                     </div>
                  </div>
               </div>
            </div>
          ...
          <!-- Section 4: Conflits -->
          <section class="bg-[#F8FAFC] p-8 rounded-[2.5rem] border-2 border-[#E2E8F0] space-y-6 animate-fade-slide-up" style="animation-delay: 0.3s">
             <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                   <ShieldCheck class="text-[#38BDF8]" :size="24" />
                   <h3 class="text-sm font-black text-[#0C2340] uppercase tracking-widest">Vérification temps réel</h3>
                </div>
                <div class="flex items-center gap-3">
                   <span class="text-[10px] font-black text-[#64A8CC] uppercase">Forcer ?</span>
          ...
          <section class="space-y-6 animate-fade-slide-up" style="animation-delay: 0.4s">
            <div class="flex items-center justify-between border-l-4 border-[#EC4899] pl-4">
               <div class="flex items-center gap-4">
                 ...
                 <h3 class="text-sm font-black text-[#0C2340] uppercase tracking-widest">Récurrence</h3>
               </div>
            ...
            <Transition name="expand">
               <div v-if="form.recurrent" class="grid grid-cols-2 gap-8 p-8 bg-[#FDF2F8] rounded-[2rem] border-2 border-dashed border-[#EC4899]/30">
                  <div class="space-y-2">
                     <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Début</label>
                     <input type="date" v-model="form.dateDebut" class="w-full bg-white border-2 border-[#BFDBFE] rounded-2xl p-4 font-bold text-sm outline-none focus:border-[#EC4899]">
                  </div>
                  <div class="space-y-2">
                     <label class="text-[10px] font-black text-[#64A8CC] uppercase tracking-widest ml-1">Fin</label>
                     <input type="date" v-model="form.dateFin" class="w-full bg-white border-2 border-[#BFDBFE] rounded-2xl p-4 font-bold text-sm outline-none focus:border-[#EC4899]">
                  </div>
               </div>
            </Transition>
          </section>
          ...
          <section class="space-y-4 animate-fade-slide-up" style="animation-delay: 0.5s">
            <div class="flex items-center gap-4 border-l-4 border-[#BFDBFE] pl-4">
              <MessageSquare class="text-[#64A8CC]" :size="20" />
              <h3 class="text-sm font-black text-[#0C2340] uppercase tracking-widest">Notes</h3>
            </div>
            <textarea v-model="form.note" rows="2" class="w-full bg-[#F0F9FF] border-2 border-[#BFDBFE] rounded-[1.5rem] p-6 text-sm font-medium text-[#1E5F8E] focus:border-[#38BDF8] outline-none resize-none" placeholder="Ajouter une consigne..."></textarea>
          </section>
        </div>
        ...
        <div class="p-8 border-t border-[#F0F7FF] flex justify-between items-center bg-[#F8FBFF] shrink-0">
          <div class="flex flex-col">
             <div class="text-[10px] font-black uppercase text-[#64A8CC] flex items-center gap-2">
          ...
          <div class="flex gap-4">
             <button @click="emit('close')" class="px-8 py-4 rounded-2xl font-black text-[#64A8CC] uppercase text-[10px] tracking-widest hover:bg-white transition-all">Annuler</button>
             <div class="relative group">
                <button
                   @click="submit"
                   :disabled="isLoading || !canSave"
                   class="min-w-[240px] px-10 py-4 bg-gradient-to-r from-[#38BDF8] to-[#0EA5E9] text-white rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-[#38BDF8]/20 hover:-translate-y-1 active:scale-95 transition-all disabled:opacity-50 disabled:translate-y-0"
                >
        ...
        <div v-if="showDeleteConfirm" class="absolute inset-0 z-[210] flex items-center justify-center p-8 bg-[#0C2340]/60 backdrop-blur-md animate-fade-in">
           <div class="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl animate-modal-in">
              <div class="w-20 h-20 bg-red-50 text-red-500 rounded-3xl flex items-center justify-center mx-auto mb-6"><Trash2 :size="40" /></div>
              <h3 class="text-xl font-black text-[#0C2340] text-center mb-2">Supprimer ce créneau ?</h3>
              <p class="text-xs text-[#64A8CC] text-center mb-8">Action irréversible pour <strong>{{ form.matiere?.nom }}</strong> le <strong>{{ form.jour }} à {{ form.heureDebut }}</strong>.</p>

              <div v-if="form.recurrent" class="grid grid-cols-1 gap-3 mb-8">
                 <button @click="deleteOption = 'only'" class="p-4 rounded-2xl border-2 font-black text-[10px] uppercase tracking-widest transition-all" :class="deleteOption === 'only' ? 'border-[#38BDF8] bg-[#F0F9FF] text-[#38BDF8]' : 'border-[#BFDBFE] text-[#64A8CC]'">Ce créneau uniquement</button>
                 <button @click="deleteOption = 'all'" class="p-4 rounded-2xl border-2 font-black text-[10px] uppercase tracking-widest transition-all" :class="deleteOption === 'all' ? 'border-[#EC4899] bg-[#FDF2F8] text-[#EC4899]' : 'border-[#BFDBFE] text-[#64A8CC]'">Tous les récurrents</button>
              </div>

              <div class="flex gap-4">
                 <button @click="showDeleteConfirm = false" class="flex-1 py-4 font-black text-[10px] uppercase text-[#64A8CC]">Annuler</button>
