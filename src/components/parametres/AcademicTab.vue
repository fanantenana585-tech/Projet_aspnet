  ...
  <div class="space-y-8 animate-in fade-in duration-500">
    ...
    <section class="bg-[#1E293B] rounded-3xl border border-gray-800 p-8 shadow-xl">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="space-y-2">
          <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Année courante</label>
          <input v-model="systeme.anneeScolaire" type="text" class="w-full bg-emit-bg/50 border border-gray-700 text-white rounded-2xl p-4 font-bold outline-none" />
        </div>
        <div class="space-y-2">
          <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Début des cours</label>
          <input v-model="systeme.semaineDebut" type="date" class="w-full bg-emit-bg/50 border border-gray-700 text-white rounded-2xl p-4 font-bold outline-none" />
        </div>
      </div>
    ...
    <section class="bg-[#1E293B] rounded-3xl border border-gray-800 p-8 shadow-xl">
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-sm text-gray-500">Définissez les périodes de cours standards.</p>
        </div>
      ...
      <div class="space-y-3">
        <div v-for="c in systeme.creneauxHoraires" :key="c.id" class="p-4 bg-gray-800/40 rounded-2xl border border-gray-800 flex items-center gap-6 group">
          <GripVertical :size="18" class="text-gray-700 group-hover:text-gray-500 cursor-grab" />
      ...
      <div class="divide-y divide-gray-800/50">
        <SettingRow label="Pause déjeuner obligatoire" description="Bloque automatiquement la plage horaire du midi.">
          <div class="flex items-center gap-4">
            <span class="text-[10px] font-bold text-gray-500">12:30 - 14:00</span>
        ...
        <SettingRow label="Volume horaire max" description="Nombre maximum d'heures d'enseignement par semaine pour un enseignant.">
          <div class="flex items-center gap-3">
            ...
            <span class="text-xs font-bold text-gray-500 uppercase">Heures</span>
          </div>
        </SettingRow>
      </div>
    </section>
